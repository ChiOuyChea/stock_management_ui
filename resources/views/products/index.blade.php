...
@section('content')
    ...
    <div class="stats-grid mb-4">
        ...
        <div class="stat-card">
            ...
            <div class="stat-info">
                <h6>Low Stock</h6>
                <div class="value">{{ count(array_filter($products, fn($p) => $p['stock'] > 0 && $p['stock'] <= 10)) }}
                </div>
            </div>
        </div>
                ...
                <table class="table table-hover mb-0" id="productsTable">
                    ...
                    <tbody id="productsBody">
                        @forelse($products as $index => $product)
                            <tr data-name="{{ strtolower($product['name']) }}" data-stock="{{ $product['stock'] }}"
                                data-id="{{ $product['id'] }}" data-price-in="{{ $product['price_in'] }}"
    ...
    <script>
        ...
        async function searchProductsAPI(searchTerm) {
            try {
                ...
                `http://localhost:3000/api/product?name=${encodeURIComponent(searchTerm)}`);
        ...
        `;
            ...
            productsBody.innerHTML = products.map((product, index) => {
                ...
                if (stock > 10) {
                    statusBadge =
                        '<span class="badge badge-success"><i class="bi bi-check-circle me-1"></i>In Stock</span>';
                } else if (stock > 0) {
                    statusBadge =
                        '<span class="badge badge-warning"><i class="bi bi-exclamation-circle me-1"></i>Low Stock</span>';
                } else {
                    statusBadge =
                        '<span class="badge badge-danger"><i class="bi bi-x-circle me-1"></i>Out of Stock</span>';
                }
